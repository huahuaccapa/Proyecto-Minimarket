...
router.get(
  ...
  (req, res) => response(res, store.purchases),
);

router.get(
  "/suppliers",

  (req, res) => response(res, store.suppliers),
);
  ...
  (req, res) => {
    ...
    if (
      store.purchases.some(
        (p) =>
          ...
          String(p.documentType || "").toLowerCase() ===
            documentType.toLowerCase() &&
          String(p.documentNumber || "").toLowerCase() ===
    ...
    if (!Array.isArray(req.body.detail) || !req.body.detail.length) {
      throw new HttpError(
        ...
        "La compra debe incluir productos",
      );
    }
    ...
    const detail = req.body.detail.map((line) => {
      ...
      if (seenProducts.has(product.id)) {
        throw new HttpError(
          ...
          `${product.name} está repetido. Déjalo en una sola fila y suma la cantidad`,
        );
      }
      ...
      return {
        ...
        expirationDateBefore: clean(product.expirationDate),
      };
    });
    ...
    addCashMovement({
      ...
      reason: `Compra ${purchase.number} - ${supplier.businessName}`,
    ...
    response(
      ...
      `Compra registrada. Se descontaron S/ ${total.toFixed(2)} de la caja`,
...
router.post(
  ...
  (req, res) => {
    const purchase = store.purchases.find((item) => item.id === req.params.id);
    ...
    for (const line of purchase.detail || []) {
      if (line.stockBefore === undefined || line.unitCostBefore === undefined) {
        throw new HttpError(
          409,
      ...
      if (!product) {
        throw new HttpError(
          ...
          `Ya no existe ${line.productName}`,
        );
      }

      const laterMovement = store.inventoryMovements.find(
        (movement) =>
      ...
      if (laterMovement) {
        throw new HttpError(
          ...
          `No se puede anular ${purchase.number}: ${line.productName} tuvo movimientos posteriores`,
        );
      }
    ...
    for (const line of purchase.detail || []) {
      ...
      product.stock = Number(Number(line.stockBefore || 0).toFixed(3));

      product.unitCost = Number(Number(line.unitCostBefore || 0).toFixed(4));

      product.purchasePrice = Number(
        line.purchasePriceBefore ?? product.purchasePrice,
      );

      product.contentQuantity = Number(
        line.contentQuantityBefore ?? product.contentQuantity,
      );

      product.purchaseQuantity = Number(
        line.purchaseQuantityBefore ?? product.purchaseQuantity,
      );

      product.lot = clean(line.lotBefore);

      product.expirationDate = clean(line.expirationDateBefore);
      ...
      store.inventoryMovements.push({
        ...
        reasonType: "anulacion_compra",

        reason: `Anulación ${purchase.number}`,

        referenceType: "purchase",

        referenceId: purchase.id,

        quantity: Number(line.units || 0),
    ...
    purchase.voidedBy = req.user.id;

    purchase.voidReason = clean(req.body.reason || "Anulación de compra");
    ...
    response(
      ...
      "Compra anulada y efectivo reintegrado a caja",
    );
  },
);
